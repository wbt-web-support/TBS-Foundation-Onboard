...
export function SaveProgressButton({
  ...
  const { flushSave, completed, submissionId, resumeToken, refId } = useOnboarding();
  ...
  const runSave = useCallback(
    async (notifyEmail: string) => {
      ...
      try {
        ...
        const effectiveRefId = saved?.refId ?? refId;
        if (!serverToken || effectiveRefId == null) {
          throw new Error(
        ...
        const resumeUrl = buildResumeUrl(effectiveRefId, window.location.origin);
        ...
        const res = await fetch("/api/save-progress-email", {
          ...
          body: JSON.stringify({
            ...
            currentStep,
            totalSteps,
            referenceId: String(effectiveRefId),
