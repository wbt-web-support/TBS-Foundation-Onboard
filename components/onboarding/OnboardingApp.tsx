...
type Action =
  ...
  | { type: "SET_IDS"; submissionId: string; resumeToken: string; refId: number | null }
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "SET_IDS":
      return { ...state, submissionId: action.submissionId, resumeToken: action.resumeToken, refId: action.refId };
...
/** `?id=` present on first page load (not added later by save-progress redirect). */
...
export default function OnboardingApp() {
  ...
  const save = useCallback(
    async (overrides?: Partial<AutosavePayload>, opts?: { force?: boolean }) => {
      ...
      try {
        ...
        if (!s.submissionId) {
          dispatch({ type: "SET_IDS", submissionId: data.id, resumeToken: data.resumeToken, refId: data.refId ?? null });
          try {
          ...
          if (data.refId != null) {
            ...
            params.set("id", String(data.refId));
            router.replace(`/?${params.toString()}`);
          }
        }
        dispatch({ type: "SET_SAVE_STATUS", status: "saved" });

        return { id: submissionId, resumeToken: data.resumeToken, refId: data.refId ?? null };
      } catch {
  ...
  useEffect(() => {
    ...
    const urlToken = urlParams.get("id") ?? searchParams.get("id");
    ...
    const hydrateFromServer = async (token: string, tokenWasInUrl: boolean) => {
      const res = await fetch(`/api/submission?id=${encodeURIComponent(token)}`);
      ...
      if (data.found && data.submission) {
        ...
        if (tokenWasInUrl && sub.refId != null && String(sub.refId) !== token) {
          const params = readClientSearchParams();
          params.set("id", String(sub.refId));
          router.replace(`/?${params.toString()}`);
        }
  ...
  return (
    <OnboardingContext.Provider value={ctx}>
      ...
      <div className="flex min-h-[calc(100vh-3.5rem)]">
        <Sidebar onBackToWelcome={() => setShowWelcome(true)} />
