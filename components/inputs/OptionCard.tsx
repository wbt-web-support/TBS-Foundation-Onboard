...
export function OptionCard({
  ...
  if (tileMulti) {
    return (
      ...
      >
        ...
        <div className="border-t border-b1 bg-panel px-2 py-2.5 text-center text-xs font-medium leading-snug text-ink">
          {label}
        </div>
      </button>
    );
  ...
  return (
    ...
    >
      {imageUrl ? (
        <div className="relative flex h-14 w-full shrink-0 items-center justify-center rounded-md bg-surface ring-1 ring-b2">
          <Image
            src={imageUrl}
            alt=""
            width={120}
      ...
      <div className={`flex min-w-0 flex-1 items-start gap-3 ${hasRichDetail ? "sm:gap-3.5" : ""}`}>
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1.5">
          <span className={hasRichDetail ? "font-semibold text-ink" : "text-ink"}>{label}</span>
          {description ? (
            <p className="text-sm leading-relaxed text-mid">{description}</p>
          ) : null}
          {warning ? <p className="text-xs font-medium leading-snug text-red">{warning}</p> : null}
          {example ? (
            <div className="rounded-md border border-b1 bg-surface px-2.5 py-2 text-sm italic leading-relaxed text-ink">
              {example}
            </div>
          ) : null}
          {linkUrl ? (
