...
function NavLink({
  ...
  return (
    ...
    >
      <Icon
        name={icon}
        className={`mt-0.5 size-5 shrink-0 ${active ? "text-brand-600" : "text-slate-400"}`}
      />
      <span className="min-w-0">
        <span className={`block text-sm font-medium ${active ? "text-brand-700" : "text-slate-700"}`}>
          {label}
        </span>
  ...
  const panel = (
    <aside className="flex h-full w-[260px] shrink-0 flex-col border-r border-slate-200 bg-white">
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">
        <p className="mb-2 px-3 text-xs font-medium uppercase tracking-wide text-slate-400">Dashboard</p>
      ...
      <div className="space-y-3 border-t border-slate-100 px-4 py-4">
        ...
        >
          <Icon name="sheet" className="size-4 text-slate-400" />
