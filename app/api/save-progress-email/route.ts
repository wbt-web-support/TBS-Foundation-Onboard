...
export async function POST(request: Request) {
  ...
  if (!referenceId || !REF_ID_RE.test(referenceId)) return jsonError("Invalid referenceId", 400);
  const idParam = resume.searchParams.get("id");
  if (!idParam || idParam !== referenceId) return jsonError("referenceId must match id in resumeUrl", 400);
