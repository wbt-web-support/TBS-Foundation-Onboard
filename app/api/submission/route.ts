...
// GET /api/submission?id=<ref_id> — load a saved submission by reference ID.
export async function GET(request: Request) {
  const params = new URL(request.url).searchParams;
  const refIdParam = params.get("id");
  const refId = refIdParam ? parseInt(refIdParam, 10) : NaN;

  if (!refIdParam || isNaN(refId) || refId <= 0) {
  ...
  const { data, error } = await supabase
    ...
    .select("id, resume_token, ref_id, answers, current_section_index, completed")
    .eq("ref_id", refId)
    .maybeSingle();
